...
# 1. 関数decode⇒DBox→BBoxに変換
# 2. confの閾値を超えたBBoxを取り出す
# 3. 関数nm_supressionを実施し、同一物体に対して被っているBBoxを削除する
...
class Detect(Function):
    ...
    def __call__(self, loc_data, conf_data, dbox_list): # forward(self, loc_data, conf_data, dbox_list):⇒これだと推論時にエラーとなる
        """
        順伝搬の計算を実行する。
        Parameters
        ----------
